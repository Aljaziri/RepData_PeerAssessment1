...
So the average is **`r mean_steps`** steps and the median is **`r median_steps`** steps.
...
The 5-minute interval containing the maximum of steps on average across all the days is the **`r max_Index`**th row, with an interval value of **`r Interval_value`** (**`r format(Hour_value, format = "%H:%M")`**), with an average of **`r round(max_Interval,0)`** steps.
...
```
In that data set, there are **`r NA_number`** NA values.
...
The average is **`r mean_steps_2`** steps and the median is **`r median_steps_2`** steps.

We notice that the average is neutral (previous value: **`r mean_steps`**), on median it's insignificant (previous value: **`r median_steps`**).
